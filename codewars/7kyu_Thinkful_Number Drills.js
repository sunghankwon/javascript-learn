...
You own a number of international warehouses that you use to store your frozen pizzas, and you need to figure out how many crates of pizzas you can store at each location.
...
The function should take three arguments: the length, width, and height of your warehouse (in feet) and should return an integer representing the number of boxes you can store in that space.
...
function boxCapacity(length, width, height) {
  ...
  for (i = 0; i < arguments.length; i++) {
    let pizzaCount = (arguments[i] * 12) / 16;
    let integer = parseInt(pizzaCount);
    sum *= integer;
  }
